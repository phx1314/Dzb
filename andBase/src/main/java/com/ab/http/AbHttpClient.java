...
public class AbHttpClient {
     ...
     * @param responseListener the response listener
     */
    public void postForm(final String url, final AbRequestParams params,
                         final AbHttpResponseListener responseListener) {
        responseListener.setHandler(new ResponderHandler(responseListener));
        responseListener.onStart();
        mExecutorService.execute(new Runnable() {
            public void run() {
                try {
                    doPostFrom(url, params, responseListener);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
     ...
     * @param responseListener the response listener
     */
    private void doPostFrom(String url, AbRequestParams params, AbHttpResponseListener responseListener) {
        try {
            if (!AbAppUtil.isNetworkAvailable(mContext)) {
                Thread.sleep(200);
                responseListener.sendFailureMessage(AbHttpStatus.CONNECT_FAILURE_CODE, AbAppConfig.CONNECT_EXCEPTION, new AbAppException(AbAppConfig.CONNECT_EXCEPTION));
            ...
            if (params != null) {
                //使用NameValuePair来保存要传递的Post参数设置字符集
                HttpEntity httpentity = params.getMultiEntity();
                //请求httpRequest
                httpPost.setEntity(httpentity);
            }
            //取得默认的HttpClient
